...
den_hab_adj <- den_nvis %>%
  ...
  mutate(den_freq=n) %>%
  select(-n) %>%
  mutate(den_perc=(den_freq/sum(den_hab_adj$den_freq))*100) %>%
  # Remove dens outside MFWS
...
den_hab_plot <- den_hab_adj %>%
  ...
  # Calculate frequency of dens adjusted by available area 
  mutate(den_freq_adj=((den_freq/area_km2)*
                         mfws_area_minus_aquatic/nrow(den_hab_adj))) %>%
  # Calculate percentage of dens adjusted by available area
  mutate(den_perc_adj=(den_freq_adj/sum(den_hab_plot$den_freq_adj))*100, 
...
den_hab_plot$mfws_habitat <- factor(den_hab_plot$mfws_habitat, 
...
```{r}
# Plot raw den percentages per habitat type
hab_raw_barplot <- ggplot(data=den_hab_plot, 
...
# Plot adjusted den percentages per habitat type
hab_adj_barplot <- ggplot(data=den_hab_plot, 
...
# Combine grazing zone and habitat type plots for publication
summary_barplots <- ggarrange(hab_barplots, zone_barplots, ncol= 1, nrow= 2, 
                                   widths= c(1,1), heights = c(1,1)) 

print(summary_barplots)
```
...
ggsave(den_summary_plot, filename="output/den summary plot.jpeg", 
       width=250, height=225, units="mm")

ggsave(summary_barplots, filename="output/grazing zone habitat summary plot.jpeg", 
       width=225, height=175, units="mm")
```
